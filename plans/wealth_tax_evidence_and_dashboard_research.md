...
### Descriptive household-type composition — 20–21 September 2026

Separately authorised by the user after the feasibility handoff. Smallest coherent implementation: one static chart using 10316, no age view, calculator expansion, national profiles or layout redesign. Reviewed the generator, notebook, tests, browser checker and export packaging first. Extend the existing offline embedding pipeline with a second named generated function rather than change the original source snapshot or require browser data fetching.

- [x] Embed separately versioned 2024 household-type means/counts from the archived feasibility source; preserve the original reference and every snapshot byte. Generator now writes only the notebook and checks the old derived artifact rather than overwriting it.
- [x] Show 15 household types in source order, four mutually exclusive asset components above zero, total debt below zero and published net wealth independently. Keep the national row separate from the groups. Derive other real assets by subtracting both housing components from real capital; retain ±100 NOK accounting discrepancies.
- [x] Explain all-household/nonowner denominators, mixed valuation, year, exclusions, rounding and statistical household versus legal tax unit. Do not load means as taxpayers, fabricate within-group distributions or calibrate population results.
- [x] Preserve full coordinated curves, wealth-decile financial-assets/residual references and official scenario context. No runtime dependency, packaging or tax-mechanics changes.
- [x] Verify all selected source cells, counts, signs, missing/invalid-data rejection and chart schema. All 23 unit tests, both generated-reference comparisons, Ruff and Marimo checks pass. Full three-notebook WASM build and Chrome reform/reset checks pass; browser checks identify the new chart's canvas and signed component data and confirm its specification stays fixed through policy changes.

**WASM compatibility: PASS.** Marimo, Polars and Altair remain the only runtime packages; all are supported by the existing WASM runtime. No runtime filesystem/network data access or unsupported process/thread pattern added. PyMuPDF remains offline-only and Playwright remains test-only.

**T2 follow-through — 21 September 2026:** bounded full/fractional ownership and qualifying joint scenarios are now specified in `docs/wealth_tax_supported_scenarios.md` and implemented/tested. See the [execution tracker and verification log](wealth_tax_autonomous_followthrough.md#completion-log): 29 unit tests, expanded real WASM ownership interactions, unchanged full-owner defaults and isolated legacy population ownership. Annual-resolution evidence, municipal/special/mixed-asset rules and population mappings remain unresolved/excluded. T1 is still pending; T3+ was not started.

**Outcome and next gate:** the authorised descriptive composition task is complete. All source definitions and model boundaries remain in force; matching wealth-decile housing/debt and national ownership/tax-unit mapping are still missing. Human review of chart density/order, labels, desktop/mobile experience and queued scope choices remains at the final consolidated gate. No bookmarks moved, pushing or integration performed.

**Autonomous follow-through completed 23 September 2026:** see the [execution tracker](wealth_tax_autonomous_followthrough.md) and [final human-review packet](wealth_tax_human_review.md) for T0–T8, exact integrated checks and acceptance questions. The property-to-tax-unit model is **explicitly illustrative**, not an evidence-backed national mapping or receipts forecast; unresolved source/legal gaps listed above remain open. Stop for human review before integration.
