...
| T3a | Population modelling contract | done | [Scenario contract](../docs/wealth_population_scenarios.md): property-level mixtures, distinct tax units, explicit assumptions, exact integration and sensitivity; document/source checks passed; no engine/UI changes |
...
| T7 | Integrated verification | done | 40 tests, Ruff, Marimo, generator, 69 hashes, three exports, final Chrome desktop/mobile interactions and T0 numerical comparison passed; see review packet |
| T8 | Human-review packet | done | [Review packet](wealth_tax_human_review.md) and master cross-reference; stop for human acceptance, no integration |

### Completion log

- T7/T8 — 23 September 2026: integrated verification after T3c, T4 and T5; [final review packet](wealth_tax_human_review.md) contains walkthrough, before/after, screenshots, results, evidence limits and numbered acceptance decisions. Final run: 40 unittest cases passed (1.840s); Ruff, Marimo check, generated-reference check passed; 69 archived source hashes including uncompressed legal handbook verified; all three WASM exports succeeded; final Chrome exported-app desktop/mobile check passed with no captured browser errors. T0 seven full-owner points and identity/10m/20m legacy weighted effects unchanged. Final target HTML 299,187 bytes versus T0 212,373; browser whole-script wall 55.85s vs T0 median 14.33s, but final script exercises much more and caches/runtime differ, not a speed-regression comparison. Within final warm Chrome run initial render 13.16s; first desktop/mobile policy update 0.96/0.95s, no pass/fail timing threshold. Saved final local desktop/mobile screenshots; no push, integration or bookmark move. Next: human review only.
...
- T3c — 23 September 2026: wired the tested scenario engine into an expandable table of four named assumption sets, with modelled reference/reform/difference totals, reconstructed property counts and assumed tax-unit counts. The 25/75 variant preserves per-property holdings; the high-assets/debt variants add 2m per expensive property and explicitly change portfolio marginals. All use the same assumed population under both policies; default common-profile result, curves, official estimates and SSB reference data remain separate. Added helper/identity/unit tests and exported-WASM browser checks for table, reform/reset, unchanged personal-share mapping and tail sensitivity. Verification: 35 unittest cases, Ruff, Marimo, generated-reference check, three WASM exports and Chrome desktop/mobile browser smoke passed (initial render 21.12s, first preset 0.99s, warm unthrottled run); actual browser execution confirmed. HTML 267,850 bytes; no new package or source fetch. Final integrated screenshots and source-hash check remain T7. Next: T4 age composition.

- T3b — 23 September 2026: added typed property-bin/template/tax-unit profiles, validated property-conditional weights and shares, fixed illustrative profiles, exact per-template/bin integration of both policies, model tax-unit counts and within-bin difference extrema in `apps/building_taxation.py`. Added `tests/test_wealth_scenarios.py` and clarified the T3a contract's implementation status. Separate co-owners use separate allowances; qualifying joint owners use one combined unit. The existing one-full-owner illustration, app controls and static sources are unchanged. Checks: 34 unit tests passed (including 5 new tests), `uv run ruff check .`, `uv run marimo check apps/building_taxation.py`, `uv run scripts/build_wealth_reference.py --check`, all three exports via `uv run .github/scripts/build.py`, and `uv run scripts/check_wealth_browser.py` desktop/mobile smoke (no captured browser errors). An ad-hoc 30-bin identity run gave 1,711,400 property-weight units, 1,886,020 **assumed** tax units and zero policy change (0.19s local Python engine-only); these are not observed tax-unit totals or browser timings. Browser smoke tests the unchanged visible UI, **not** a displayed scenario result; T3c must test that integration. Initial browser render measured 32.89s under this run's local conditions; no causal comparison or threshold inferred. No new external package, source snapshot, live fetch, bookmark move or integration. Next ready task: T3c compact UI (independent T4 also ready); human review remains T8.

- T3a — 23 September 2026: added `docs/wealth_population_scenarios.md` and updated this tracker. Specified reconstructed 2026 property-bin units and unknown assumed tail; separated published sources, derived counts and freely assumed property-conditional profiles. Defined tax-unit multiplicity/allowance semantics, a small illustrative debt/assets/ownership mix, fixed-population policy comparison, exact piecewise integration, conditional extrema and correlation sensitivity. No engine, dashboard, source snapshot or dependencies changed. Verification: reviewed relevant source reports, existing calculator/population helper and tests; `uv run python` checked all 7 local document links and embedded 30-bin totals (1,711,400 overall; 34,800 at 14–30m); reviewed JJ diff/status. Documentation-only: no unit suite, WASM export or browser run claimed. Next ready task: T3b engine/test implementation; independent T4 remains ready. Human visual/acceptance review stays T8.
